...
MOO
;
print "<table class=\"buildstatus\" cellspacing=\"1\" cellpadding=\"2\"><tr><th>Timestamp</th>\n";
for my $t (sort titlesort keys %title) {
...
foreach my $l (reverse sort @logs) {
	...
	my $date = $l;
	$date =~ s/^sizes-//;
        $b =~ s/sizes-(\d\d\d\d)(\d\d)(\d\d)T(\d\d)(\d\d)(\d\d)Z/$1-$2-$3 $4:$5/g;
        $b = "<a class=\"bstamp\" href=\"/cvsmod/chlog-$date.html\">$b</a>";
