...
for(reverse sort keys %date) {
    ...
    print "<p><i>Please note that producing these files takes some time, and if you visit this page while builds are in progress you may see it only partially updated.<br/> Please check back later if what you want is missing, or use the 'older builds' links to grab something from a previous daily build.</i>\n";
