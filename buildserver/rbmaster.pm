...
sub getspeed($)
{
    ...
    my $rows = $getspeed_sth->execute($cli, 20);
    if ($rows > 0) {
        my @ulspeeds;
        my @buildspeeds;
...
sub db_prepare
{
    ...
    $getspeed_sth = $db->prepare("SELECT id, timeused, ultime, ulsize FROM builds WHERE client=? AND id NOT LIKE '%manual%' AND errors = 0 AND warnings = 0 ORDER BY time DESC LIMIT ?") or
        warn "DBI: Can't prepare statement: ". $db->errstr;
